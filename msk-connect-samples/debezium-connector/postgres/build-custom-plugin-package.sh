...
download_unzip_base_files(){
  debezium_zip io/debezium $DEBEZIUM_CONNECTOR_NAME $DEBEZIUM_CONNECTOR_VERSION
  unzip -q -j $DOWNLOAD_FILE -d $DOWNLOAD_FILE_TMP_PATH
  echo "Removing $DOWNLOAD_FILE"
  mkdir -p .archive
  mv $DOWNLOAD_FILE .archive/

  msk_config_providers_zip r${MSK_CONFIG_PROVIDER_VERSION} msk-config-providers-${MSK_CONFIG_PROVIDER_VERSION}-with-dependencies.zip
  unzip -q -j $DOWNLOAD_FILE -d $DOWNLOAD_FILE_TMP_PATH
  echo "Removing $DOWNLOAD_FILE"
  mkdir -p .archive
  mv $DOWNLOAD_FILE .archive/
...
export DEBEZIUM_CONNECTOR_NAME="debezium-connector-postgres"
export DEBEZIUM_CONNECTOR_VERSION="2.2.0.Final"
export MSK_CONFIG_PROVIDER_VERSION="0.2.0"
export DOWNLOAD_FILE_TMP_PATH="./${DEBEZIUM_CONNECTOR_NAME}"
